...
// La carte Leaflet n'est construite qu'une fois la bibliothèque chargée
// (voir ensureLeaflet() dans viewswitch.js, appelé quand la section approche du viewport).
let map = null;
let marker = null;
...
function initLeafletMap() {
  ...
  marker = L.marker([CURRENT_LOCATION.lat, CURRENT_LOCATION.lng], { icon: pulseIcon, zIndexOffset: 1000 })
    .addTo(map)
    .bindPopup(`📍 <strong>${CURRENT_LOCATION.name}</strong><br>Position actuelle`);

  const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  if (reduceMotion) {
    // Pas d'animation cinématique : positionnement direct sur le lieu actuel.
    map.setView([CURRENT_LOCATION.lat, CURRENT_LOCATION.lng], MY_ZOOM);
    marker.openPopup();
  } else {
...
function leafletZoomToMe() {
  if (!map) return;
  const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  map.flyTo([CURRENT_LOCATION.lat, CURRENT_LOCATION.lng], MY_ZOOM, { duration: reduceMotion ? 0 : 1.6 });
  marker.openPopup();
}
function leafletZoomToMada() {
  if (!map) return;
  const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  map.flyTo([MADAGASCAR_VIEW.lat, MADAGASCAR_VIEW.lng], MADAGASCAR_VIEW.zoom, { duration: reduceMotion ? 0 : 1.6 });
}
